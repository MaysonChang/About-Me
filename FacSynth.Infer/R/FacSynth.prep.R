...
#' @description  This function extracts relevant data objects from the given dataframe and produce a list of matrices necessary for running the factor model approach.
...
#' @export
#' @return A list with a series of components prepared for running \code{FacSynth.infer}.
...
#'      )
#'
#'   ## Second example: The effect of Election-Day Registration reforms
#'   ## on voter turnout in the United States, which is sourced from
#'   ## Package "gsynth".
#'
#'   #load data from the package.
#'   data(turnout_wide)
...
#'      controls.identifier = c(2:6, 8:10),
#'      time.variable = "year",
#'      time.prior = seq(1920, 1972, by = 4),
#'      time.post = seq(1976, 2012, by = 4)
#'      )
#'

FacSynth.prep = function(data = NULL,treatment.identifier = NULL,
                      ...
                      time.prior = NULL, time.post = NULL)
{
  ...
  CCC = matrix(1,nrow(X),1)
  X = as.matrix(cbind(CCC,X))
