...
#' @details Users are recommended to supply data object obtained by running the \code{tsSynth.prep}, or import data of both treated and control units over the pretreatment periods manually. Moreover, a subsampling method serves to yield consistent estimators of the test statistic, and thus users need to choose appropriate subsample size and times of subsampling repetition.
#' @param synth.prep.obj The object that comes from running \code{tsSynth.prep}. This objects contains matrices necessary for running this function. Therefore, once users import this object, there is no need to specify Y1, X1 manually.
  ...
  if (pJ >0.05) {
    ...
    if (pa > 0.05 & pb < 0.05) {
      cat("\n****************", "\n****************",
          ...
          "\n\n Recommend to apply the MSCa method (with weights sum to one).\n\n")
    }
      ...
      cat("\n****************", "\n****************",
          ...
          "\n p-value for 'weights sum to one' hypothesis:",pa,
          "\n p-value for 'without intercept' hypothesis:",pb,
          "\n\n Recommend to apply the MSCb method (with zero intercept).\n\n")
    }
    else if (pa < 0.05 & pb < 0.05) {
      cat("\n****************", "\n****************",
          ...
          "\n p-value for 'weights sum to one' hypothesis:",pa,
          "\n p-value for 'without intercept' hypothesis:",pb,
          "\n\n Recommend to apply the MSC method (with non-negativity).\n\n")
    }
  }
}
