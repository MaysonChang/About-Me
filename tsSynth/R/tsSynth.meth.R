...
#' @param synth.prep.object The object that comes from running \code{synth.prep}. This objects contains all information about four necessary matrices. Therefore, once users import this object, there is no need to specify any of four matrices manually.
...
#' @param INTCP A logical flag. If true, an intercept term will be added as an independent variable. Noted that this argument is available only when argument \code{meth = "modified"}.
#' @param wgt.sum A logical flag. If true, weights that derived from chosen SC method sum to one. Similarly, this argument is available only when argument \code{meth = "modified"}.
...
#' \item{ATT_S}{The value of estimated average treatment effect on treated unit.}
...
#'   #appropriate.
#'   #now run tsSynth.meth function to apply original synthetic model
...
#'      wgt.sum = TRUE)
#'
#'   #or users can choose MSC model with weights sum to one.
#'   tsSynth.meth.out2 =
#'     tsSynth.meth(
#'      tsSynth.prep.out,
#'      meth = "modified",
...
#'      wgt.sum = TRUE)
#'
#'   #or apply MSC model with zero intercept.
#'   tsSynth.meth.out3 =
#'     tsSynth.meth(
#'      tsSynth.prep.out,
#'      meth = "modified",
...
#'      wgt.sum = FALSE)
#'
#'   #or apply MSC model as estimation method.
#'   tsSynth.meth.out4 =
#'     tsSynth.meth(
#'      tsSynth.prep.out,
#'      meth = "modified",
...
tsSynth.meth = function(tsSynth.prep.obj = NULL, meth = "ordinary", INTCP = FALSE, wgt.sum = TRUE,
                        Y1 = NULL, X1 = NULL, Y2 = NULL,X2 = NULL, time.variable = NULL, time.prior = NULL,
                        time.post = NULL)
{
  ...
  if (meth == "ordinary") {
    ...
    n = ncol(X1)
      ...
      stop("\n Argument 'INTCP' is a logical flag. \n")
    }
    lb0 = matrix(0, n, 1)
    lb0[1] = -Inf
    b0 = matrix(1, n, 1)/n
    ...
    else if (wgt.sum = FALSE) {
      res = fmincon(b0, fn, lb = lb0)
    }
    else {
      stop("\n Argument 'wgt.sum' is a logical flag. \n")
    }
